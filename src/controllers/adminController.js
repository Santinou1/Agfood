...
const { sendBulkMail } = require("../utils/mailService");
const renderMessageAdmin = require("../utils/renderMessageAdmin");

const sendBulkMailNotification = () => {
  return Usuario.find({}, 'mail')
    .then(usuarios => {
      const emailList = usuarios.map((user) => user.mail);
      
      return sendBulkMail(emailList, {
...
const processExcelUpload = (req, res) => {
  ...
  new Promise((resolve, reject) => {
    ...
    .then(() => {
      ...
      return sendBulkMailNotification();
    })
    .catch((error) => {
